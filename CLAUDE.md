...
- For debugging: DDEV supports Xdebug out of the box. Enable it via `ddev xdebug on` and configure your IDE (e.g., VS Code with Xdebug extension) to listen on the mapped port.
- Environment variables: Managed via `.env` file, which DDEV loads automatically into the container.
- File syncing: DDEV uses Mutagen for fast file syncing between host and container; ensure it's installed and running for optimal performance.
...
- Run browser tests in DDEV with `ddev artisan test` (Pest handles browser setup via Dusk or similar integrations).
